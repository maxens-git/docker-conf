...
sudo docker run authelia authelia crypto rand --length 72 --charset rfc3986

sudo docker run authelia/authelia:latest authelia crypto rand --length 64 --charset alphanumeric
sudo docker run authelia authelia crypto rand --length 64 --charset alphanumeric
```
## Générer des certificats pour openid connect
```
sudo docker run --rm -u "$(id -u):$(id -g)" -v "$(pwd)":/keys authelia/authelia:latest authelia crypto pair rsa generate --directory /keys
```
